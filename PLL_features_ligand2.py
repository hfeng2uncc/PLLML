...
import warnings
import featurization_ligand2 as featurization
import pickle
from itertools import combinations

# get features of atom groups of different element types in ligand. PLL is performed on these the original whole molecule. Features are based on statistics of each atom group.

warnings.filterwarnings("ignore")

main_dir ="/users/hfeng2/projects/local_laplacian"
...
class PLL_Feature():
    ...
    def PLL_features_ES(self):
        ...
        rips_dict = pc.generate_rips_dict_by_all_distances(self.filtration_dists, max_dim=self.max_dim)
...
def save_feature_PLL(args, list_compound_id, filtration_dists):
    ...
    for pdbid in list_compound_id:
        ...
        for dim in range(args.max_dim):
            save_path = f'{args.feature_path}/{pdbid}/{pdbid}_L{dim}-fil{args.filtration_upper}_ligand2.pkl'
...
def main(args):
    ...
    filtration_radius = np.arange(
        2, args.filtration_upper + args.dr, args.dr
    )

    compounds = (
                open(
                    f"{main_dir}/datasets/{args.dataset_name}/{args.dataset_name}-{args.task}.txt"
                )
                .read()
                .splitlines()
            )

    if len(compounds) % args.unit == 0:
        num = int(len(compounds) / args.unit)
    if len(compounds) % args.unit != 0:
        num = int(len(compounds) / args.unit) + 1

    if args.isplit < num - 1:
        istart = args.isplit * args.unit
        iend = (args.isplit + 1) * args.unit
    elif args.isplit == num - 1:
        istart = args.isplit * args.unit
        iend = len(compounds)

    list_compound_id = compounds[istart:iend]
    
    save_feature_PLL(args, list_compound_id, filtration_radius)
    
    # print(f"done with {pdbid}")
    t1 = time.time()
...
def parse_args():
    ...
    parser.add_argument("--pdb_path", type=str,default="/scratch/hfeng2/datasets")
    parser.add_argument("--unit", type=int)
    parser.add_argument("--isplit", type=int)
